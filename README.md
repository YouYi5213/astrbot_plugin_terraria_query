...
- **宝藏袋** — 掉落表（物品 / 几率 / 数量）；专家专属行高亮（如 `泰拉 史莱姆王宝藏袋`）；`泰拉 宝藏袋` 列出全部
...
| 指令 | 说明 |
|------|------|
| `泰拉查询 <名称>` / `泰拉 <名称>` | 查询物品、Boss、宝藏袋、NPC、生物群系或事件（中文名称搜索，中文卡片展示） |
| `泰拉更新` | 从 Wiki **增量**更新离线数据（物品、坐骑/宠物、群系/事件、Boss、宝藏袋、NPC、缺失图片等；与 `python prepare_data.py` 相同） |
